...
export function ClassicGuideDocument({guide}:{guide:ModelGuide}) {return <Document title={`${guide.title} Painting Guide`} author={guide.author} creator="Model by Numbers"><GuideCoverPage guide={guide}/><GuideProjectPage guide={guide}/><GuideModelViewsPage guide={guide}/>{(guide.references?.length??0)>0?<GuideReferencesPage references={guide.references??[]}/>:null}<GuidePalettePage guide={guide}/><GuidePartsPage guide={guide}/></Document>}
