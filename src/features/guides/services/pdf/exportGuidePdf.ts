...
import type {ExportGuidePdfOptions,PdfExportSuccess,PdfExportWarning} from "./types";
...
export async function exportGuidePdf(options:ExportGuidePdfOptions):Promise<PdfExportSuccess>{
  ...
  const warnings:PdfExportWarning[]=imageResult.failed.length?[{code:"IMAGE_LOAD_FAILED",count:imageResult.failed.length}]:[];
