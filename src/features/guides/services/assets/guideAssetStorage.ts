...
export const guideAssetStorage = {
  async save(input: SaveGuideAssetInput): Promise<GuideAssetReference> {
    ...
    const now = new Date();
    await saveGuideAssetFile({ id, entity: "guide-asset", entityId: input.projectId, fileName: `${input.kind}-${input.assetId}`, mimeType: input.blob.type || "image/png", blob: input.blob, size: input.blob.size, createdAt: existing?.createdAt ?? now, updatedAt: now, metadata: { kind: input.kind, assetId: input.assetId } });
    return { id, storageKey: id, mimeType: input.blob.type || "image/png", createdAt: (existing?.createdAt ?? now).toISOString(), kind: input.kind, assetId: input.assetId };
  },
  ...
  async list(projectId: string): Promise<GuideAssetReference[]> {
    return (await loadGuideAssetFiles(projectId)).map(file => ({ id: file.id, storageKey: file.id, mimeType: file.mimeType, createdAt: file.createdAt.toISOString(), kind: String(file.metadata?.kind) as GuideAssetReference["kind"], assetId: String(file.metadata?.assetId) }));
  },
};
