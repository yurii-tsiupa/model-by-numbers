...
export function GuideNavigation({sections,locale}:{sections:readonly GuideSectionMetadata[];locale:Locale}){
  ...
  return <>
    <details data-guide-navigation className="group mx-5 mt-6 rounded-2xl border border-white/10 bg-neutral-900/90 p-4 sm:mx-6 lg:hidden"><summary className="flex cursor-pointer list-none items-center justify-between font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-400"><span>{t("guide.navigation.contents")}</span><ChevronDown className="h-4 w-4 transition group-open:rotate-180"/></summary>{links}</details>
    <nav data-guide-navigation aria-label={t("guide.navigation.label")} className="sticky top-24 hidden max-h-[calc(100vh-7rem)] overflow-y-auto rounded-2xl border border-white/10 bg-neutral-900/85 p-4 shadow-xl shadow-black/20 backdrop-blur lg:block"><h2 className="font-semibold text-white">{t("guide.navigation.contents")}</h2>{links}</nav>
  </>;
}
