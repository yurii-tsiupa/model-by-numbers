...
export function GuideModelOverviewManager({ locale, targetMode, views, editorHref, onChange, onCapture }: Props) {
  ...
  return <section data-guide-controls className="pt-[18px] text-[var(--text)]">
    ...
    <div className="mt-3 space-y-2">
      {ordered.map((view, index) => <article key={view.id} className="grid min-w-0 grid-cols-[3.5rem_minmax(0,1fr)_auto] items-center gap-2.5 rounded-xl border border-[var(--border)] bg-[var(--card)] p-2.5">
        <img src={view.image} alt="" className="size-14 shrink-0 rounded-lg bg-white object-contain" />
        <div className="min-w-0">
          <p className="truncate text-[13px] font-semibold">{label(view.type)}</p>
          <p className="mt-0.5 text-[11px] text-[var(--text-secondary)]">{t(view.source === "automatic" ? "guide.overviewViews.automatic" : "guide.overviewViews.custom")}</p>
        </div>
    ...
    <details className="group relative mt-2.5">
      <summary className="guide-add-action flex min-h-11 w-full cursor-pointer list-none items-center justify-center gap-1.5 px-3">
        <Plus className="size-3.5" aria-hidden="true" />{t("guide.overviewViews.addClean")}
      </summary>
      <div className="absolute bottom-11 left-0 z-20 w-full rounded-lg border border-[var(--border)] bg-[var(--card)] p-1 shadow-lg">
