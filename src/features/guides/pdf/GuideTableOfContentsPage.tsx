...
export function GuideTableOfContentsPage({ viewModel }: { viewModel: GuideViewModel }) {
  const { locale, sections, guide } = viewModel;
  const t = (key: Parameters<typeof translate>[1]) => translate(locale, key);
  return <GuidePage locale={locale} projectName={guide.title}>
    <Text style={guidePdfStyles.eyebrow}>{t("guide.cover.document")}</Text>
