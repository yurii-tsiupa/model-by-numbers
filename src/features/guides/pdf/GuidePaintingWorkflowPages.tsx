  ...
  return (
    <>
      {paintingPages.map((page) => (
        <GuidePage key={page.steps[0]?.step.id ?? "painting-empty"} backgroundSectionId="paintingInstructions" id={page.pageIndex === 0 ? "paintingInstructions" : undefined} locale={locale} pageNumber={pageNumberStart+page.pageIndex} projectName={guide.title} totalPages={totalPages} wrap={false}>
          {page.pageIndex === 0 ? (
            <PrintSectionStart>
