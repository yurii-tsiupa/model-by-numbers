...
const styles = StyleSheet.create({
  ...
  step: { fontSize: 9, color: pdfColors.accent, fontWeight: tokens.weightBold },
  ...
  overviewPartsTitle: { color: pdfColors.accent, fontFamily: tokens.headingFont, fontSize: tokens.sizeBody, fontWeight: tokens.weightSemibold, marginBottom: tokens.spacingXs },
  ...
  overviewNumber: { color: pdfColors.accent, fontFamily: tokens.monoFont, fontSize: tokens.sizeCaption, width: 28 },
...
export function GuideAssemblyPages({ pageNumberStart, totalPages, viewModel }: { pageNumberStart: number; totalPages: number; viewModel: GuideViewModel }) {
  ...
  return <>{Array.from({ length: pageCount }, (_, pageIndex) => {
    ...
    return <GuidePage key={pageIndex} id={pageIndex === 0 ? "assembly" : undefined} locale={locale} pageNumber={pageNumberStart + pageIndex} projectName={guide.title} totalPages={totalPages}>
      ...
      <Text style={styles.overviewPartsTitle}>{t("guide.assembly.modelParts")}</Text>
      <View style={styles.overviewParts}>{pageParts.map((part) => <View key={part.id} style={styles.overviewPart} wrap={false}><Text style={styles.overviewNumber}>{String(part.number).padStart(2, "0")}</Text><Text style={styles.overviewName}>{part.name}</Text></View>)}</View>
