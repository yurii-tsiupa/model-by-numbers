...
export function GuideContentPageBranding({ geometry }: { geometry: GuidePageGeometry }) {
  ...
  for (const element of ELEMENT_ORDER) {
    const settings = branding.contentPagesLayout[element];
  ...
  return <>
    {[...groups.entries()].map(([position, elements]) => {
      ...
      return <View fixed key={position} style={slotStyle(position, geometry)}>
        {elements.map((element, index) => <View key={element} style={{ alignItems: alignment === "left" ? "flex-start" : alignment === "right" ? "flex-end" : "center", marginTop: index ? 3 : 0, maxWidth: "100%" }}>
          {element === "logo" && branding.logoUrl ? <Image src={branding.logoUrl} style={{ height: 18, objectFit: "contain", width: 44 }} /> : null}
          ...
          {element === "qr" && qrImageUrl ? <View style={{ backgroundColor: "#FFFFFF", padding: 3 }}><Image src={qrImageUrl} style={{ height: 42, width: 42 }}/></View> : null}
        </View>)}
      </View>;
    })}
  </>;
