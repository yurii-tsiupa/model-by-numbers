...
export function buildGuidePaintingStepViewModels(guide:ModelGuide):GuidePaintingStepViewModel[]{
  const locale=guide.locale??"en",parts=guide.workflowParts??guide.parts,partById=new Map(parts.map(part=>[part.id,part])),details=guide.manualDetails??[],detailById=new Map(details.map(detail=>[detail.id,detail])),palette=guide.previewPalette??guide.workflowPalette??guide.palette;
  let order=0;
