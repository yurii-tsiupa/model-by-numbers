...
export function ModelImportReport({ analysis, warnings, onChooseAnother }: { analysis: ModelAnalysis; warnings: ModelImportWarning[]; onChooseAnother: () => void }) {
  ...
  const rows: Array<[string, string]> = [
    [t("modelImport.report.format"), t(`modelImport.formats.${analysis.format ?? "glb"}`)],
    ...
    [t("modelImport.report.materials"), analysis.capabilities?.supportsMaterials === false ? t("modelImport.capabilities.notAvailable") : formatLocalizedNumber(analysis.materials.materialsCount, locale)],
    [t("modelImport.report.textures"), analysis.capabilities?.supportsTextures === false ? t("modelImport.capabilities.notAvailable") : formatLocalizedNumber(analysis.textures.texturesCount, locale)],
  ...
  async function copyReport() {
    await navigator.clipboard.writeText(createTechnicalImportReport({ analysis, format: analysis.format ?? "glb", userAgent: navigator.userAgent }));
  }
