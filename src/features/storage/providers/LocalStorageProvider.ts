...
function toFile(entity: StorageEntity, record: LegacyRecord): StorageFile | null {
  ...
  const fileName = String(entity === "guide" ? record.fileName : entity === "reference" ? record.name : entity === "assembly-step-image" ? record.fileName : `${entityId}-thumbnail`);
...
function toRecord(file: StorageFile): LegacyRecord {
  ...
  if (file.entity === "assembly-step-image") return { ...base, id: file.id, projectId: file.entityId, fileName: file.fileName, mimeType: file.mimeType, blob: file.blob, size: file.size };
...
export class LocalStorageProvider implements StorageProvider {
  ...
  async get(id: string) { const db=await openLocalDatabase(); for (const entity of ["reference","guide","thumbnail","assembly-step-image"] as const) { const record=await result(db.transaction(STORAGE_STORES[entity]).objectStore(STORAGE_STORES[entity]).get(id) as IDBRequest<LegacyRecord|undefined>,"read"); if(record) return toFile(entity,record); } return null; }
