...
const previewPrimaryClass=`inline-flex min-h-9 cursor-pointer items-center justify-center gap-1.5 rounded-lg border border-[var(--accent)] bg-[var(--accent)] px-3 text-xs font-semibold text-[var(--accent-foreground)] shadow-sm transition hover:brightness-110 active:scale-[.98] disabled:cursor-not-allowed disabled:opacity-40 ${previewFocusClass}`;
...
export function StepPreview({projectId,step}:{projectId:string;step:PaintingStage}){
  ...
  return <section className="mt-3 rounded-lg border border-[var(--border)] bg-[var(--bg)] p-3">
    <div className="flex items-center justify-between gap-2">
      <div><h4 className="text-xs font-semibold">{regionDetails.length?t("editor.region.previewTitle"):t("editor.steps.preview.title")}</h4><p className="text-[10px] text-[var(--text-secondary)]">{regionDetails.length?t("editor.region.previewHelp"):t("editor.steps.preview.automatic")}</p></div>
      <div className="flex flex-wrap justify-end gap-2">
        {!overviewEnabled?<button type="button" onClick={restoreOverview} className={previewSecondaryClass}>{t("editor.steps.previewShots.restoreOverview")}</button>:null}
        {targetDetails.length?<button ref={addButtonRef} type="button" onClick={()=>setChoosing(true)} className={previewPrimaryClass}><Plus className="size-3"/>{regionDetails.length?t("editor.region.addCustomPreview"):t("editor.steps.previewShots.addAnother")}</button>:null}
      </div>
    </div>
    {previewError?<p role="alert" className="mt-2 text-xs text-red-400">{t("editor.region.previewCameraUnavailable")}</p>:null}
    {overviewEnabled||shots.length?<div className="mt-3 grid gap-3 sm:grid-cols-2">
      ...
      {shots.map((shot,index)=>{const detail=details.find(item=>item.id===shot.manualDetailId);const pinIndex=shot.type==="manualDetailLocation"?detail?.pins.findIndex(pin=>pin.id===shot.pinId)??-1:-1;const unavailable=!detail||(shot.type==="manualDetailLocation"?pinIndex<0:detail.targetMode!=="region"||!detail.region?.selections.length);const label=shot.type==="manualDetailRegion"&&detail?t("editor.region.customPreviewLabel",{name:detail.name,index:index+1}):detail&&pinIndex>=0?t("editor.steps.previewShots.detailLabel",{number:detail.number,name:detail.name,index:pinIndex+1}):t("editor.steps.previewShots.unavailable");return <PreviewImage key={shot.id} projectId={projectId} step={step} shot={shot} label={label} title={title} unavailable={unavailable} onReplace={shot.type==="manualDetailRegion"?()=>replaceRegion(shot):undefined} onRemove={()=>removeShot(shot)}/>})}
    </div>:null}
...
function PreviewImage({projectId,step,shot,label,title,unavailable=false,onReplace,onRemove}:{projectId:string;step:PaintingStage;shot?:PaintingStepPreviewShot;label:string;title:string;unavailable?:boolean;onReplace?:()=>void;onRemove?:()=>void}){
  ...
  return <figure className="overflow-hidden rounded-lg border border-[var(--border)] bg-[#F7F7F5]">
    <div className="flex min-h-10 items-center justify-between bg-[var(--card)] px-2"><figcaption className="truncate text-[10px] text-[var(--text-secondary)]">{label}</figcaption><div className="flex gap-1"><button type="button" disabled={loading||unavailable} onClick={onReplace??(()=>void generate(true))} title={onReplace?t("editor.region.replacePreview"):t("editor.region.generatePreview")} aria-label={onReplace?t("editor.region.replacePreview"):t("editor.region.generatePreview")} className={`grid size-8 cursor-pointer place-items-center rounded-md border border-[var(--border)] bg-[var(--card)] text-[var(--accent)] transition hover:border-[var(--accent)] hover:bg-[var(--bg)] active:scale-95 disabled:cursor-not-allowed disabled:opacity-40 ${previewFocusClass}`}><RefreshCw className={`size-3.5 ${loading?"animate-spin":""}`}/></button>{onRemove?<button type="button" onClick={onRemove} aria-label={t("editor.steps.previewShots.accessibility.remove",{label})} className={`grid size-8 cursor-pointer place-items-center rounded-md border border-red-500/20 text-red-500 transition hover:bg-red-500/10 active:scale-95 ${previewFocusClass}`}><Trash2 className="size-3.5"/></button>:null}</div></div>
    <div className="relative aspect-[3/2]">{!hasTargets?<div className="grid h-full place-items-center p-3 text-center text-xs text-neutral-500">{t("editor.steps.preview.general")}</div>:unavailable?<div className="grid h-full place-items-center p-3 text-center text-xs text-neutral-500">{t("editor.steps.previewShots.missingLocation")}</div>:loading?<div role="status" className="grid h-full place-items-center text-xs text-neutral-500">{t("editor.steps.preview.generating")}</div>:result?<Image unoptimized fill sizes="320px" src={result.imageUrl} alt={shot?t("editor.steps.previewShots.accessibility.alt",{title,label}):t("editor.steps.preview.accessibility.alt",{title})} className="object-contain"/>:<button type="button" onClick={()=>void generate(true)} className={`absolute inset-3 m-auto h-fit ${previewPrimaryClass}`}>{error?t(`editor.steps.preview.errors.${error}`):t("editor.region.generatePreview")}</button>}</div>
  </figure>
}
