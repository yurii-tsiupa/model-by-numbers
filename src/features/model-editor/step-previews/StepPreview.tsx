  ...
  return <section className="mt-2 rounded-xl border border-[var(--border)] bg-[var(--card)] p-2.5">
    <div>
      ...
      <div className="mt-2 flex items-center justify-start gap-1.5">
        ...
        {targetDetails.length?<button ref={addButtonRef} type="button" onClick={()=>setChoosing(true)} className={`${previewPrimaryClass} w-fit px-2.5`}><Plus className="size-3.5 shrink-0"/><span>{regionDetails.length?t("editor.region.addCustomPreview"):t("editor.steps.previewShots.addAnother")}</span></button>:null}
      </div>
    </div>
