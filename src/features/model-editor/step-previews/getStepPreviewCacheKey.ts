...
export function getStepPreviewCacheKey(projectId:string,step:PaintingStage,parts:readonly {id:string;meshUuid?:string;paletteColorId?:string|null}[],manualDetails:readonly ManualDetail[],palette:readonly Pick<PaletteColor,"id"|"hex">[]=useModelEditorStore.getState().palette,shot?:PaintingStepPreviewShot){
 ...
 return JSON.stringify({v:STEP_PREVIEW_RENDERER_VERSION,projectId,stepId:step.id,shot:shot?shot.type==="manualDetailLocation"?{id:shot.id,type:shot.type,manualDetailId:shot.manualDetailId,pinId:shot.pinId,pin:shotPin?{position:shotPin.position,normal:shotPin.normal,camera:shotPin.camera,updatedAt:shotPin.updatedAt}:null}:{id:shot.id,type:shot.type,manualDetailId:shot.manualDetailId,camera:shot.camera}:"overview",refs:references.map(reference=>{if(reference.type==="part"){const part=partById.get(reference.id);return{...reference,mesh:part?.meshUuid,color:part?.paletteColorId?colors.get(part.paletteColorId):null}}const detail=detailById.get(reference.id);return{type:"manualDetail",id:reference.id,number:detail?.markerNumber??detail?.number,updatedAt:detail?.updatedAt,targetMode:detail?.targetMode??"markers",region:detail?.region,color:detail?.colorId?colors.get(detail.colorId):null,pins:detail?.pins.map(pin=>({id:pin.id,position:pin.position,normal:pin.normal,camera:pin.camera,updatedAt:pin.updatedAt}))}})});
}
