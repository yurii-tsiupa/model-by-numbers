...
export const useModelEditorStore =
  create<ModelEditorState>()((set) => ({
    ...
    deletePaintingStage:(partId,stageId)=>set((state)=>{const part=state.parts.find(p=>p.id===partId);if(!part?.paintingWorkflow.stages.some(s=>s.id===stageId))return state;return{parts:state.parts.map(p=>p.id===partId?{...p,paintingWorkflow:{...p.paintingWorkflow,stages:p.paintingWorkflow.stages.filter(s=>s.id!==stageId).map((s,i)=>({...s,order:i+1}))}}:p),activePaintingStageId:state.activePaintingStageId===stageId?null:state.activePaintingStageId,...markStateDirty(state)}}),
