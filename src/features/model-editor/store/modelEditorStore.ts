...
export const useModelEditorStore =
  create<ModelEditorState>()((set) => ({
    ...
    duplicatePaintingStage:(partId,stageId)=>set((state)=>{const part=state.parts.find(p=>p.id===partId),index=part?.paintingWorkflow.stages.findIndex(s=>s.id===stageId)??-1;if(!part||index<0)return state;const source=part.paintingWorkflow.stages[index],now=new Date().toISOString(),duplicate={...source,id:crypto.randomUUID(),targetReferences:source.targetReferences?.map(reference=>({...reference})),createdAt:now,updatedAt:now},stages=[...part.paintingWorkflow.stages.slice(0,index+1),duplicate,...part.paintingWorkflow.stages.slice(index+1)].map((stage,i)=>({...stage,order:i+1}));return{parts:state.parts.map(p=>p.id===partId?{...p,paintingWorkflow:{...p.paintingWorkflow,stages}}:p),activePaintingStageId:duplicate.id,...markStateDirty(state)}}),
