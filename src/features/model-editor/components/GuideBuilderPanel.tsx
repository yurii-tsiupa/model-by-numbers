...
export function SimplePalettePanel({captureHidden=false}:{captureHidden?:boolean}) {
  const { t } = useTranslation();
  return <aside inert={captureHidden||undefined} aria-hidden={captureHidden||undefined} data-onboarding-target={ONBOARDING_TARGETS.projectPalette} className={`order-2 w-full min-w-0 shrink-0 border-t border-[var(--border)] bg-[var(--card)] lg:order-1 lg:h-full lg:w-[340px] lg:overflow-x-hidden lg:overflow-y-auto lg:border-r lg:border-t-0 ${captureHidden?"pointer-events-none":""}`}>
...
export function GuideBuilderPanel(props: ReferenceProps) {
  const { t } = useTranslation();
  return <aside inert={props.captureHidden||undefined} aria-hidden={props.captureHidden||undefined} data-onboarding-target={ONBOARDING_TARGETS.paintingSteps} className={`simple-steps-panel order-3 flex max-h-[38rem] min-h-0 w-full min-w-0 shrink-0 flex-col overflow-hidden border-t border-[var(--border)] lg:h-full lg:max-h-none lg:w-[400px] lg:border-l lg:border-t-0 ${props.captureHidden?"pointer-events-none":""}`}>
