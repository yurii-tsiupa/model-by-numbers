...
import { PaintingStageFormModal } from "./painting/PaintingStageFormModal";
import { MarkerDetailsSection } from "./MarkerDetailsSection";
...
export function GuideBuilderPanel({ onOpenGuide, canOpenGuide }: { onOpenGuide: () => void; canOpenGuide: boolean }) {
  ...
  const addPaintingStage = useModelEditorStore((state) => state.addPaintingStage);
  const selectedPart = parts.find((part) => part.id === selectedPartId) ?? null;
  const readiness = useMemo(() => getGuideBuilderReadiness(parts, palette, markers), [markers, palette, parts]);
  ...
  return <aside className="flex max-h-[26rem] min-h-0 w-full shrink-0 flex-col overflow-hidden border-t border-[var(--border)] bg-[var(--card)] lg:h-full lg:max-h-none lg:w-80 lg:border-l lg:border-t-0"><header className="shrink-0 border-b border-[var(--border)] p-4"><h2 className="font-[family-name:var(--font-space-grotesk)] text-base font-semibold">{t("editor.guideBuilder.title")}</h2><p className="mt-1 text-xs text-[var(--text-secondary)]">{t("editor.guideBuilder.description")}</p><nav aria-label={t("editor.accessibility.builderSections")} className="mt-3 grid grid-cols-4 gap-1">{SECTIONS.map((item, index) => { const Icon = item.icon; const active = section === item.id; return <button key={item.id} type="button" aria-current={active ? "step" : undefined} onClick={() => setSection(item.id)} className={`flex min-w-0 flex-col items-center gap-1 rounded-lg px-1 py-2 text-[10px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] ${active ? "bg-[var(--accent)] text-[var(--accent-foreground)]" : "text-[var(--text-secondary)] hover:bg-[var(--bg)]"}`}><span aria-hidden="true" className="flex items-center gap-1"><span>{index + 1}</span><Icon className="size-3.5" /></span><span className="max-w-full truncate">{t(`editor.guideBuilder.sections.${item.id}`)}</span></button>; })}</nav></header><div className="min-h-0 flex-1 overflow-y-auto p-4">
    ...
    {section === "steps" ? <section><h3 className="text-sm font-semibold">{t("editor.steps.title")}</h3><p className="mt-1 text-xs leading-5 text-[var(--text-secondary)]">{t("editor.steps.description")}</p>{parts.length > 0 ? <select aria-label={t("editor.accessibility.detailSelect")} value={selectedPartId ?? ""} onChange={(event) => selectPart(event.target.value)} className="mt-3 h-10 w-full rounded-lg border border-[var(--border)] bg-[var(--bg)] px-3 text-sm"><option value="" disabled>{t("editor.colors.selectDetail")}</option>{parts.map((part) => <option key={part.id} value={part.id}>{part.name}</option>)}</select> : null}{!selectedPart || selectedPart.paintingWorkflow.stages.length === 0 ? <p className="mt-5 text-sm text-[var(--text-secondary)]">{t("editor.steps.empty")}</p> : <ol className="mt-3 space-y-2">{selectedPart.paintingWorkflow.stages.map((stage) => <li key={stage.id} className="flex gap-3 rounded-lg border border-[var(--border)] p-3"><span className="font-[family-name:var(--font-jetbrains-mono)] text-xs text-[var(--text-secondary)]">{String(stage.order).padStart(2, "0")}</span><div className="min-w-0"><p className="text-sm font-medium">{getPaintingStageLabel(stage, t)}</p>{stage.notes ? <p className="mt-1 text-xs leading-5 text-[var(--text-secondary)]">{stage.notes}</p> : null}</div></li>)}</ol>}<button type="button" disabled={!selectedPart} onClick={() => setShowStageForm(true)} className="mt-3 flex min-h-10 w-full items-center justify-center gap-2 rounded-lg bg-[var(--accent)] px-3 text-sm text-[var(--accent-foreground)] disabled:opacity-40"><Plus className="size-4" />{t("editor.steps.add")}</button>{showStageForm && selectedPart ? <PaintingStageFormModal stage={null} palette={palette} onClose={() => setShowStageForm(false)} onSave={(input) => { addPaintingStage(selectedPart.id, input); setShowStageForm(false); }} /> : null}</section> : null}
