...
export function SimplePartColorDetail({stageId,partId,colorId,onChange}:{stageId:string;partId:string|null;colorId:string|null;onChange:(partId:string|null,colorId:string|null)=>void}){
  ...
  function choosePart(nextPartId:string){
    const nextColorId=Object.hasOwn(stepDraftAssignments,nextPartId)?stepDraftAssignments[nextPartId]:null;
  ...
  function chooseNoColor(){
    if(!partId)return;
    clearAssignments([partId]);
    setDraft({stageId,partId,paletteColorId:null});
  ...
  return <section className="space-y-3">
    ...
    <button type="button" disabled={!partId} aria-pressed={colorId===null} onClick={chooseNoColor} className={`simple-palette-color-card flex min-h-[44px] w-full cursor-pointer items-center gap-3 rounded-lg border bg-[var(--card)] px-3 py-2 text-left text-xs disabled:cursor-not-allowed disabled:opacity-40 ${colorId===null?"border-[var(--accent)] ring-1 ring-[var(--accent)]":"border-[var(--border)]"} ${focusClass}`}><span className="grid size-7 shrink-0 place-items-center rounded-lg border border-dashed border-[var(--border-strong)] bg-transparent text-[var(--text-muted)]">—</span><span className="font-semibold text-[var(--text)]">{t("painting.stage.noColor")}</span>{colorId===null?<Check className="ml-auto size-4 text-[var(--accent)]"/>:null}</button>
    <div>
      ...
      <div className="mt-1.5 flex min-h-10 items-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--card)] px-3">
        <span className={`size-5 shrink-0 rounded-md border ${selectedColor?"border-black/15":"border-dashed border-[var(--border-strong)] bg-transparent"}`} style={selectedColor?{backgroundColor:selectedColor.hex}:undefined}/>
        <span className="min-w-0 truncate text-xs font-medium">{selectedColor?.name??t("painting.stage.noColor")}</span>
      </div>
    </div>
    <div>
