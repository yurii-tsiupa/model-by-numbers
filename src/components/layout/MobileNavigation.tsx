...
export function MobileNavigation({ open, onClose, navigation, pathname, user, showRegistration }: { open: boolean; onClose: () => void; navigation: readonly NavigationItem[]; pathname: string; user: User | null; showRegistration: boolean }) {
  ...
  return <div className="fixed inset-0 z-[60] bg-[var(--bg)] text-[var(--text)] md:hidden" role="dialog" aria-modal="true" aria-label={t("header.mobileMenu")}>
    ...
    {user?<nav aria-label={t("profile.userMenu.label")} className="mt-auto flex flex-col gap-1 border-t border-[var(--border)] p-4"><Link href="/profile" onClick={onClose} className="rounded-xl px-4 py-3 text-sm font-medium">{t("profile.userMenu.profile")}</Link><Link href="/profile/account" onClick={onClose} className="rounded-xl px-4 py-3 text-sm font-medium">{t("profile.userMenu.account")}</Link><SignOutButton/></nav>:null}
