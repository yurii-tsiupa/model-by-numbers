...
export function MobileNavigation({ open, onClose, navigation, pathname, user }: { open: boolean; onClose: () => void; navigation: readonly NavigationItem[]; pathname: string; user: User | null }) {
  ...
  return <div className="fixed inset-0 z-[60] bg-[var(--bg)] text-[var(--text)] md:hidden" role="dialog" aria-modal="true" aria-label={t("header.mobileMenu")}>
    ...
    <div className="mt-auto flex flex-wrap items-center gap-3 border-t border-[var(--border)] p-4"><LanguageSwitcher/><ThemeToggle/>{user ? <SignOutButton/> : <Link href="/login" onClick={onClose} className="inline-flex min-h-10 items-center rounded-[10px] bg-[var(--accent)] px-4 text-sm font-medium text-white">{t("header.login")}</Link>}</div>
  </div>;
}
