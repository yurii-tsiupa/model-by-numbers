...
export default function GuidesPage() {
  ...
  return <ProtectedAppShell><GuidesLibraryHeader guideCount={library.guides.length} modelCount={groups.length}/><div className="mt-5 flex flex-col gap-3 sm:flex-row"><label className="min-w-0 flex-1 text-sm text-[var(--text)]"><span className="sr-only">{t("guides.search.label")}</span><input type="search" value={search} onChange={event=>setSearch(event.target.value)} placeholder={t("guides.search.placeholder")} className="h-10 w-full rounded-[10px] border border-[var(--border)] bg-[var(--card)] px-3 text-sm outline-none focus-visible:border-[var(--accent)] focus-visible:ring-2 focus-visible:ring-[var(--accent)]"/></label><label className="text-sm"><span className="sr-only">{t("guides.sort.label")}</span><select value={sort} onChange={event=>setSort(event.target.value as ModelGuideGroupSort)} className="h-10 w-full cursor-pointer rounded-[10px] border border-[var(--border)] bg-[var(--card)] px-3 text-sm outline-none focus-visible:border-[var(--accent)] focus-visible:ring-2 focus-visible:ring-[var(--accent)] sm:w-auto"><option value="recent">{t("guides.sort.recent")}</option><option value="oldest">{t("guides.sort.oldest")}</option><option value="alphabetical">{t("guides.sort.alphabetical")}</option></select></label></div><section className="mt-5">{library.isLoading?<GuidesLoadingState/>:library.isError?<GuidesErrorState onRetry={()=>void library.refetch()}/>:library.guides.length===0?<GuidesEmptyState hasModels={library.projectCount>0}/>:visibleGroups.length===0?<p role="status" className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-6 text-center text-sm text-[var(--text-secondary)]">{t("guides.search.noResults")}</p>:<div className="space-y-3">{visibleGroups.map((group,index)=><ModelGuideGroup key={group.project.id} group={group} defaultExpanded={index===0} expandForSearch={Boolean(search.trim())}/>)}</div>}</section></ProtectedAppShell>;
}
